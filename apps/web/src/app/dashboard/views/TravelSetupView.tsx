...
Please call the scout_travel_plans tool with these details.`;
  ...
  return (
    <div className="w-full h-full bg-slate-50 flex flex-col items-center justify-center p-6 md:p-20 relative overflow-hidden min-h-screen font-sans">
      
      {/* Background Ambience */}
      <div className="absolute top-0 left-0 w-full h-full opacity-[0.05] pointer-events-none">
         <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-indigo-500 blur-[120px] rounded-full" />
         <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-cyan-500 blur-[120px] rounded-full" />
      </div>
      ...
      <div className="w-full max-w-3xl space-y-12 relative z-10">
        ...
        <div className="space-y-4 text-center md:text-left">
           <motion.div 
             ...
             className="inline-flex items-center gap-2 bg-indigo-600/10 border border-indigo-600/20 px-3 py-1 rounded-full"
           >
             <Palmtree className="w-3 h-3 text-indigo-600" />
             <span className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">Planning Phase</span>
           </motion.div>
           
           <div className="space-y-1">
              <h1 className="text-3xl md:text-5xl font-black text-slate-900 italic tracking-tighter uppercase leading-none">
                Destination: {destination}
              </h1>
              {includedStops.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3 mb-1">
                  {includedStops.map(stop => (
                    <span key={stop} className="px-2 py-0.5 bg-indigo-600/5 border border-indigo-600/10 rounded text-[8px] font-black text-indigo-600/60 uppercase tracking-widest">
                      Incl. {stop}
                    </span>
                  ))}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="space-y-10">
            ...
            <div className="space-y-6">
               <label className="flex items-center gap-3 text-xs font-black text-slate-900 uppercase tracking-widest">
               ...
               <div className="space-y-4">
                  <div className="flex justify-between items-end">
                     ...
                     <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Days</span>
                  </div>
                  <input 
                    type="range" 
                    min="1" 
                  ...
                  {PACE_OPTIONS.map((opt) => (
                    ...
                    >
                      <div>
                        ...
                        <p className={`text-[9px] font-bold mt-1 uppercase ${pace === opt.id ? 'text-indigo-100' : 'text-slate-400'}`}>
                          {opt.desc}
                        </p>
                      </div>
                      {pace === opt.id && <div className="w-2 h-2 rounded-full bg-white shadow-[0_0_10px_rgba(255,255,255,0.5)]" />}
                    </button>
                  ))}
               </div>
            </div>
          ...
          <div className="space-y-6">
                 ...
                 <button 
                   ...
                   className={`px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border-2 ${
                     interests.includes(interest)
                      ...
                      : 'bg-white border-slate-200 text-slate-400 hover:border-rose-500/30'
                   }`}
                 >
                   {interest}
                 </button>
            ...
            <div className="p-8 rounded-3xl bg-indigo-600/5 border border-indigo-600/10 flex flex-col items-center justify-center text-center space-y-4 mt-8">
               <Coffee className="w-8 h-8 text-indigo-600/30" />
        ...
        <div className="pt-10 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6">
           <div className="flex items-center gap-4">
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                Planning your trip to <span className="text-slate-900 font-black italic">{destination}</span>
              </p>
           </div>
           ...
           <motion.button 
             ...
             className="w-full md:w-auto flex items-center justify-center gap-3 bg-indigo-600 text-white px-12 py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-2xl hover:bg-indigo-700 transition-all disabled:opacity-50 shadow-indigo-600/20"
           >
             {isGenerating ? (
